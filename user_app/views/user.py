...
from auth.helpers import generate_access_jwt_token, owner_required
...
async def get_token(user: UserAuth, authorize: AuthJWT = Depends()):
    ...
    return (
        generate_access_jwt_token(authorize, str(user.get("_id")), user.get("email"), user.get("hashed_pass"))
        if user
        else JSONResponse(
            status_code=404,
